{
	"categories":[
		{
		"category":"Seasonal Drinks",
        "items":[
			{
			"name":"Carib-bean",
			"info":"Coconut + Caramel Iced Latte",
			"prices":{
				"regular":4.79}
			},
			{
			"name":"Soy Dream",
			"info":"Soymilk + Tangerine + OJ + Vanilla Pudding Mix, Blended",
			"prices":{
				"regular":4.79}
			},
			{
			"name":"Pina Paradise",
			"info":"Vanilla Bean Frappuccino + Coconut Pineapple",
			"prices":{
				"tall":3.55,
				"grande":4.10,
				"venti":4.45
			}}
		]},
		{
        "category":"Frapps + Frozen",
        "items":[
            {
            "name":"Frappuccinos",
            "flavors":[
			"Coffee","Caramel","Mocha","Vanilla Bean","Strawberries & Creme"
            ],
            "prices":{
            	"regular": 4.79}
            },
            {
            "name":"Frozen Drinks",
            "flavors":[
			"Cookies & Creme", "Grasshopper", "Frozen Hot Chocolate"
            ],
            "prices":{
            	"regular":4.79}
        	}
        ]
		},
		{
		"category":"Coffee",
		"items":[
			{
			"name":"Coffee",
			"prices":{
				"tall":1.75,
				"grande":1.95,
				"venti":2.25}
			},
			{
			"name":"Cappuccino",
			"prices":{
				"tall":2.95,
				"grande":3.65,
				"venti":3.95}
			},
			{
			"name":"Café Latté",
			"prices":{
				"tall":2.95,
				"grande":3.65,
				"venti":3.95}
			},
			{
			"name":"Flavored Café Latté",
			"flavors":[
				"Almond", "Blood Orange", "Butterscotch", "Cherry", "Caramel", "Cherry-Lime", "Chocolate",
				"Coconut", "Cinnamon", "Lychee", "Creme De Menthe", "Hazelnut", "Irish Cream", "Mango",
				"Peanut Butter", "Peppermint", "Pineapple", "Pomegranate", "Raspberry", "Red Sangria",
				"Root Beer", "Strawberry", "Tangerine", "Tiramisu", "Toasted Marshmallow", "Toffee Nut",
				"Vanilla", "White Chocolate", "Caramel (SF)", "Chocolate (SF)", "Hazelnut (SF)", "Irish Creme (SF)",
				"Raspberry (SF)", "Vanilla (SF)"
			],
			"prices":{
				"tall":3.45,
				"grande":3.95,
				"venti":4.45
			}
			},
			{
			"name":"Cafe Mocha",
			"prices":{
				"tall":3.45,
				"grande":4.15,
				"venti":4.45
			}
			},
			{"name":"White Chocolate Mocha",
			"prices":{
				"tall":3.65,
				"grande":4.35,
				"venti":4.65
			}
			},
			{
			"name":"Caramel Macchiato",
			"prices":{
				"tall":3.65,
				"grande":4.25,
				"venti":4.65
			}
			},
			{
			"name":"Cafe Americano",
				"prices":{
					"tall":1.95,
					"grande":2.55,
					"venti":2.95
				}
			},
			{"name":"Cafe Au Lait",
			"prices":{
				"tall":2.05,
				"grande":2.25,
				"venti":2.45
			}
			},
			{"name":"Red Eye (1 Shot)",
			"prices":{
				"tall":2.35,
				"grande":2.65,
				"venti":2.95
			}
			},
			{"name":"Black Eye (2 Shot)",
			"prices":{
				"tall":3.05,
				"grande":3.35,
				"venti":3.65
			}
			},
			{"name":"Green Eye",
			"prices":{
				"tall":3.75,
				"grande":4.05,
				"venti":4.35}
			},
			{"name":"Double Espresso",
			"prices":{
				"regular":1.99}
			}
		]
		},
		{
		"category":"Non-Coffee Drinks",
		"items":[
			{"name":"Chai Latté",
			"prices":{
				"tall": 3.15,
				"grande": 3.65,
				"venti": 3.95}
			},
			{"name":"Dirty Chai Latté (1 Shot)",
			"prices":{
				"tall": 3.55,
				"grande": 4.25,
				"venti": 4.55}
			},
			{"name":"Matcha Green Tea Latté",
			"prices":{
				"tall": 2.95,
				"grande": 3.65,
				"venti": 3.95}
			},
			{"name":"Tazo Tea",
			"flavors":{
				"herbal":[
					"Passion", "Wild Sweet Orange", "Calm", "Chamomile", "Refresh Mint"
				],
				"black":[
					"Organic Chai", "Earl Grey", "Awake", "English Breakfast"
				],
				"green":[
					"Lotus",
					"Zen"
				]
			},
			"prices":{
				"tall": 1.50,
				"grande": 1.50,
				"venti": 1.50}
			},
			{"name":"Tazo Tea Latté",
			"flavors":{
				"herbal":[
					"Passion", "Wild Sweet Orange", "Calm", "Chamomile", "Refresh Mint"
				],
				"black":[
					"Organic Chai", "Earl Grey", "Awake", "English Breakfast"
				],
				"green":[
					"Lotus",
					"Zen"
				]
			},
			"prices":{
				"tall": 2.95,
				"grande": 3.65,
				"venti": 3.95}
			},
			{"name":"Hot Chocolate",
			"prices":{
				"tall": 2.75,
				"grande": 3.25,
				"venti": 3.45}
			},
			{"name":"White Hot Chocolate",
			"prices":{
				"tall": 2.85,
				"grande": 3.35,
				"venti": 3.45}
			},
			{"name":"Italian Soda",
			"prices":{
				"tall": 2.00,
				"grande": 2.50,
				"venti": 3.00}
			}
			]
		},
		{"category":"Sandwiches",
		"items":[
			{"name":"Croissant Sandwich",
			"info": "with Sausage, Egg + American Cheese",
			"prices":{
				"regular": 3.39}
			},
			{"name":"Bagel Sandwich",
			"info": "with Sausage, Egg + American Cheese",
			"prices":{
				"regular": 3.39}
			},
			{"name":"Muffin Sandwich",
			"info": "with Ham, Egg + American Cheese",
			"prices":{
				"regular": 3.39}
			},
			{"name":"Biscuit Sandwich",
			"info": "with Bacon, Egg + American Cheese",
			"prices":{
				"regular": 3.39}
			},
			{"name":"Kaiser Roll Sandwich",
			"info": "with Chicken, Sausage, Egg + American Cheese",
			"prices":{
				"regular": 3.39}
			},
			{"name":"Steak Bagel Sandwich",
			"info": "with Steak, Egg + American Cheese",
			"prices":{
				"regular": 4.39}
			},
			{"name":"Vegan Burrito",
			"info": "Scrambled Tofu with Tempeh Bacon, Peppers, Onions, Tomato + Vegan Cheese on a Flour Tortilla",
			"prices":{
				"regular": 4.39}
			},
			{"name":"Prosciutto Croissant",
			"info": "with Cranberry Tarragon Goat Cheese, Lettuce + Prosciutto",
			"prices":{
				"regular": 5.49}
			},
			{"name":"Margarita",
			"info": "Baguette with Mozzerella Cheese, Fresh Basil, Tomato + Balsamic Vinegar",
			"prices":{
				"regular": 5.49}
			},
			{"name":"Scone Sandwich",
			"info": "Cheddar Cheese Scone with Cheddar Cheese, Egg + Chicken Sausage",
			"prices":{
				"regular": 4.39}
			},
			{"name":"Chorizo Sandwich",
			"info": "Kaiser Roll with Chorizo, Pepperjack, Cheese + Egg",
			"prices":{
				"regular": 4.74}
			}
		]
		}			
	]
}